    ...
    if state['type'] == 'tick':
        ...
        for p in players:
            if p not in list(Players.keys()):
                ...
                Players[p].update(dict.fromkeys(['posit'],list(posit)))
                Players[p].update(dict.fromkeys(['modul'],list(modul)))
                    ...
                    for p in list(Players.keys()):
                        if p != 'i':
                            if poi in Players[p]['terri']:
                                ...
                                break
                    if chk == True:
                        points +=5 #награда не обязательно равна реальной стоимости клетки
                    else:
                        points +=1
                for nit in nitro:
                    if nit in fill[0]:
...
while True:
    ...
    if S[2] == '':
        ...
        try:
            ...
            cmd = random.choice(valid_turns(Players['i']['posit'],Players['i']['direc'],'',[],wx,wy))
            points = 0
